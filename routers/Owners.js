...
import OwnerSchema from '../mogodb/model/Owners.js'
dotenv.config();
...
router.route('/').post(async(req,res)=>{
  ...
  try {
    ...
    // const data=await OwnerSchema.findOneAndUpdate({nameId:id},{$inc:{name:req.body.name,photo:req.body.photo}})
    const fil=await OwnerSchema.findOne({nameId:nameId})
    ...
    const photoUrl=(await cloudinary.uploader.upload(photo,{folder:'owner'})).secure_url
    const data= new OwnerSchema({photo:photoUrl,name:name,nameId:nameId})
    await data.save()
...
router.route('/:id').get(async(req,res)=>{
  try {
   const id= req.params.id;
    const data=await OwnerSchema.findOne({nameId:id})
    if(data){
...
router.route('/delete/:id').delete(async(req,res)=>{
  try {
    ...
    const id=req.params.id;
    console.log(id);
    const data1=await OwnerSchema.findOne({nameId:id})
    if(data1){
    const data = await OwnerSchema.deleteOne({nameId:id})
    if(data){
