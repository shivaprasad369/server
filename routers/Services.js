...
import ServicesSchema from '../mogodb/model/Services.js'
dotenv.config();
...
router.route('/').post(async(req,res)=>{
  ...
  try {
    ...
    // const data=await ServicesSchema.findOneAndUpdate({nameId:id},{$inc:{name:req.body.name,photo:req.body.photo}})
    const fil=await ServicesSchema.findOne({nameId:nameId})
    ...
    const photoUrl=(await cloudinary.uploader.upload(photo,{folder:'ServicesPage'})).secure_url

    const data= new ServicesSchema({photo:photoUrl,name:name,nameId:nameId})
    await data.save()
...
router.route('/:id').get(async(req,res)=>{
  try {
   const id= req.params.id;
    const data=await ServicesSchema.findOne({nameId:id})
    if(data){
...
router.route('/delete/:id').delete(async(req,res)=>{
  try {
    ...
    const id=req.params.id;
    console.log(id);
    const data1=await ServicesSchema.findOne({nameId:id})
    if(data1){
    const data = await ServicesSchema.deleteOne({nameId:id})
    if(data){
